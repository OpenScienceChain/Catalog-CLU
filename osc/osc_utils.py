...
def add_files(res_json, fout):
    ...
    fout.write(
        "# Current list of contributed files. You can modify the list (to add files start with '- ')\n"
    )
    fout.write("Files:\n")
...
def add_manifest(res_json, fout):
    fout.write(
        "# List of the current files and their respective hashes. Do not modify any information below\n"
    )
    fout.write("Manifest:\n")
    manifest_lst = res_json["manifest"]
    for i in manifest_lst:
...
def update_summary(new_files, old_files, updated_files, deleted_files, id):
    ...
    print(
        "Please check 'changes.txt-"
        + id
        + "' for details and then continue with the update operation."
    )
